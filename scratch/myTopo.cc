...
public:
  ...
  void ReadFlowInput()
  {
    ...
    if (flow_type == Hadoop) {
      group = rand() % 9;
      flow_input.tenant = group + rand () % (10 - group - 1) + 1;
      flow_input.tenant += group * 10;
    }
    else {
      group = rand() % 10;
      flow_input.tenant = rand() % (group + 1);
      flow_input.tenant += group * 10;
    }
  ...
  void ScheduleFlowInput()
  {
    ...
    /*if (sip == 167867138)
      cout<<"my set ip "<<sip<<" "<<flow_type<<endl;*/
    app->Setup(
    ...
    if (flow_type == WebSearch) {
      /*if (flow_port.count(src_ip) == 0)  
        flow_port[src_ip] = 1;
      uint16_t src_port = flow_port[src_ip] + 49152;
      flow_port[src_ip]++;*/
      // uint32_t dst_ip = interfaces[flow_input.dst].GetAddress(1,0).Get();
      // uint32_t dst_port = flow_input.dport;
      stringstream ss;
...
ThroughputMonitor(FlowMonitorHelper *fmhelper, Ptr<FlowMonitor> monitor)
{
       ...
       i != stats.end(); ++i)
  {
    ...
    if (localThrou != 0)
    {
      ...
      std::ofstream thr3("vPIFOResult/firstTime.dat",
  ...
  stringstream path1;
  path1 << "vPIFOResult/performance.dat"; // plotResult
  stringstream path2;
  path2 << "vPIFOResult/performance_detail.dat"; // plotResult
