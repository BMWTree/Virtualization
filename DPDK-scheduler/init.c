...
app_init_rings(void) {
    ...
    app.ring_flows_size = (topower2(app.buff_size_bytes / MEAN_PKT_SIZE) << 2);
    for (i = 0; i < 6; i++) {
        char name[32];

        snprintf(name, sizeof(name), "app_ring_flows_%u", i);

        app.rings_flows[i] = rte_ring_create(
            name,
            app.ring_flows_size,
            rte_socket_id(),
            RING_F_SP_ENQ | RING_F_SC_DEQ);

        if (app.rings_flows[i] == NULL)
            rte_panic("Cannot create ring_flows_%u\n", i);
    }

    app.ring_nodes_size = (topower2(app.buff_size_bytes / MEAN_PKT_SIZE) << 2);
    for (i = 0; i < 2; i++) {
        char name[32];

        snprintf(name, sizeof(name), "app_ring_nodes_%u", i);

        app.rings_nodes[i] = rte_ring_create(
            name,
            app.ring_nodes_size,
            rte_socket_id(),
            RING_F_SP_ENQ | RING_F_SC_DEQ);

        if (app.rings_nodes[i] == NULL)
            rte_panic("Cannot create ring_flows_%u\n", i);
    }
}
